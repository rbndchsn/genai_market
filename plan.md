...
| Current phase | Phase 5 complete; MVP live at https://rbndchsn.github.io/genai_market/. Phase 6 (chat) is paused. Next is Phase 7 (QA and launch). |
| Next step | 7.1b (optional) hand-review the 30 records that share eight-word runs with the public sources (mostly chart labels); then 7.2 accuracy spot-check of 20 stats and insights against their cited pages. |
...
| 2026-09-16 | The Sources page no longer lists the JSON data files for download. The files stay in `site/data/` because every page reads them at run time, but the site does not advertise them. | User question: why hand the whole dataset to visitors? A download list invites wholesale copying of our own research output with no attribution, no data licence has been stated, two of the files are build artifacts, and the vendor records derive from a licensed report. The Method section already says the site is data-driven. If reuse is wanted later, add an explicit data licence line instead. |
| 2026-09-16 | The Sources page no longer has a "How to cite" section. Stable record ids and links stay. | User concern: a citation format projects more confidence in our interpretation than a meta-analysis of other people's surveys should claim, especially before the 7.2 accuracy pass, and it pulls readers one step away from the underlying sources. Attribution is already covered under Licensing; the site-as-a-whole line duplicated the footer. |
| 2026-09-16 | The site is show-only, not a collaboration. The Corrections paragraph (open an issue in the repository) and the footer Repository link are removed; no page links to GitHub. | User decision. A public issue channel invites public challenges to our figures and points visitors at the repository, which should not be evident. The github.io address still names the repo; only a custom domain (open in 7.4) would change that. |
