...
| Next step | 5.10 Deploy to GitHub Pages: add `.github/workflows/pages.yml` (upload `site/` as the Pages artifact, deploy), enable Pages with the workflow build type, confirm https://rbndchsn.github.io/genai_market/ renders every page, record the URL. |
...
**Layout.** Static files only, no build step, all paths relative (the site will live under `/genai_market/` on GitHub Pages). Pages: `index.html`, `insights.html`, `vendors.html`, `stats.html`, `glossary.html`, `sources.html`, `404.html`. Each page has the same head block (see any existing page: viewport, description, favicon, `css/site.css`, an inline script that applies a stored theme before first paint), a `<body data-page="key">` (keys: home, insights, vendors, stats, glossary, sources) and a `<main id="main" class="container">`. Every page loads `js/shell.js` first and its own `js/<page>.js` after. Placeholder bodies use `<div class="placeholder">` and are replaced step by step.
