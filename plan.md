...
| Next step | 5.5 `stats.html`: charts from `stats.json` grouped by theme, one chart per stat record following its `chart` hint (bar, grouped-bar, donut, range, table, none as stat tile), each with sample, note and source line, a table view per chart, hover tooltips, filters by theme and source in the query string, deep links `#s4-02`. Load `dataviz` guidance first (already loaded this session: thin bars capped at 24px, 2px surface gaps, one hue for single series, at most eight fixed categorical slots, direct labels selectively, table twin for every chart). |
