...
| Next step | 7.1b (optional) hand-review the 30 records that share eight-word runs with the public sources (mostly chart labels); then 7.2 accuracy spot-check of 20 stats and insights against their cited pages. Show-only site (decided 2026-09-16): no data downloads, no citation section, no corrections channel, no repository link; do not reintroduce them. Browsers cache `js/` and `css/` for ten minutes after a deploy (Pages `max-age=600`); a version stamp on the script and stylesheet links was offered and not requested. |
