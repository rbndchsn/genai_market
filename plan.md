...
| Next step | 5.4 `vendors.html`: sortable and filterable table from `vendors.json` (quadrant, Pe and Ma scores and ranks, HQ, industries), interactive PeMa quadrant chart (SVG, 28 points, threshold 0.5, hover tooltip, direct labels for Leaders), profile drawer with focus, platforms, differentiators, claimed metrics marked unaudited, partners and refs. Load `dataviz` guidance before the chart (all-pairs form: at most three categorical hues, so colour quadrants with one hue plus emphasis or use shape). |
...
**Review routine, run before marking any step done.** `python scripts/review_shots.py "<page>?<query>#<hash>" [--height N] [--dark] [--out DIR]` starts a local server, screenshots the page at 1280px and inside a 390px iframe (desktop Chrome cannot shrink below about 500px, so the iframe is the phone view), prints the file paths and the rendered DOM size. Open both images with the Read tool and check: nothing overflows horizontally, no label collisions, data actually rendered (DOM size in the thousands), the active nav item is right, dark and light both work (`--dark`, or the theme toggle stores `theme` in localStorage), deep links open their target. For DOM-level checks use the browser's `--dump-dom` with `--virtual-time-budget=6000` and grep. `node --check site/js/<file>.js` for syntax. Fix every defect found before recording the step.
...
**Decisions still open for the user at the start of 5.4:** Q4 site name (GenAI Market until told otherwise); Q5 vendor coverage (recommendation: all 28 in table and quadrant, drawer for each); whether to run 5.4 to 5.9 without stopping.
