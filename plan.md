...
| Current phase | Phase 7 (QA and launch) in progress; site live at https://rbndchsn.github.io/genai_market/ with nine pages including "How it was built" (7.0d). Phase 6 (chat) is paused. The build method is published on purpose (decision 2026-09-16). |
| Next step | 7.3 technical QA (Lighthouse performance and accessibility; links and console are already checked by `check_site.py`). 7.1b (optional public-source overlap review) remains available. Show-only site (decided 2026-09-16): no data downloads, no citation section, no corrections channel, no repository link; do not reintroduce them. Browsers cache `js/` and `css/` for ten minutes after a deploy (Pages `max-age=600`); a version stamp on the script and stylesheet links was offered and not requested. |
