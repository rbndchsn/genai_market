...
| Next step | 5.8 Search: `scripts/build_search_index.py` writes `site/data/search-index.json` (compact inverted index over `chunks.json` plus glossary expansions); `search.html` with `?q=` in the URL, results grouped with type pills, highlighted snippets and links to the record anchors; a Search entry in the shell nav and a header search form on wide screens. |
